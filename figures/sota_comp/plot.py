...
def plot_miou(dict_mious, ls="-", color="tab:blue", label=None, marker='^', unit=10, ms=1.0):
...
def plot_model(model, ls='-', color="tab:blue", marker='^', ms=5.0, unit=10):
    ...
    plot_miou(dict_mious, label=f"{model.split('/')[-1]}", ls=ls, color=color, unit=unit, marker=marker,  ms=ms)
...
if __name__ == '__main__':
    ...
    if DATASET == "cv":
        ...
        plot_model(f"{DATASET}/FPN50", color="g", unit=unit, marker='s')
        plot_model(f"{DATASET}/FPN101", color="b", unit=unit, marker='p')
